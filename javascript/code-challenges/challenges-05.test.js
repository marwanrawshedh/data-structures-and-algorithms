...
const toLastNames = people => {
  let newarr=people.map(elemnt=>{
return `${elemnt.firstName} ${elemnt.lastName}`
...
const addValues = (arr) => {
  // Solution code here...
  let newarr=arr.reduce((ss,element,x)=>{
  ss=ss+element
return ss},0
  )
  return newarr
...
const addPurchases = (arr) => {
  let newarr=arr.reduce((ss,elemnt,x)=>{
    ss=ss+elemnt.purchasePrice
  return ss},0
    )
    return newarr
};
...
const reversedString = (str) => {
  let arr = str.split("");
  let newarr = arr.reduce((ss, element) => {
    ss = element + ss + "";
    return ss;
  }, []);
  return newarr;
